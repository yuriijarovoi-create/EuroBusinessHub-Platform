/**
 * Fetch all place=* settlements in Rheinland-Pfalz from OpenStreetMap (Overpass API).
 * Output: scripts/osm-rp-places.json
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const OUT = path.join(path.dirname(fileURLToPath(import.meta.url)), 'osm-rp-places.json');
...
area["name"="Rheinland-Pfalz"]["boundary"="administrative"]["admin_level"="4"]->.rp;
(
  node["place"="city"]["name"](area.rp);
  node["place"="town"]["name"](area.rp);
  node["place"="village"]["name"](area.rp);
  node["place"="hamlet"]["name"](area.rp);
);
out body;
`;
...
[out:json][timeout:300];
(
  node["place"="city"]["name"](49.05,6.05,50.95,8.55);
  node["place"="town"]["name"](49.05,6.05,50.95,8.55);
  node["place"="village"]["name"](49.05,6.05,50.95,8.55);
  node["place"="hamlet"]["name"](49.05,6.05,50.95,8.55);
);
out body;
`;
...
try {
  ...
  console.error('Area query failed — falling back to RP bbox');
