/**
 * Cross-state integrity + duplicate checks after SH import.
 */
import fs from 'fs';
...
function countMoselle() {
  const rpPath = path.join(ROOT, STATE_FILES['Rheinland-Pfalz']);
  if (!fs.existsSync(rpPath)) return 0;
  const rp = fs.readFileSync(rpPath, 'utf8');
  let mosel = 0;
